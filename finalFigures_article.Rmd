---
title: ''
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

```{r packages, eval=FALSE, include=FALSE}
library(tableone)
```

# Appendix

## Model 1

```{r echo=FALSE}
knitr::include_graphics("./CC2/sinjer/ppc_plot_sinjer_mort.png")

```
**Figure 1.** Posterior predictive distribution density plot for deaths.
<br>
<br>
<br>
<br>


```{r echo=FALSE}
knitr::include_graphics("./CC2/sinjer/ppc_plot_sinjer_hosp.png")

```
<br>
<br>
<br>
<br>


```{r echo=FALSE}
knitr::include_graphics("./CC2/sinjer/beta_intervalssinjer.png")

```
<br>
<br>
<br>
<br>


```{r echo=FALSE}
knitr::include_graphics("./CC2/sinjer/beta_h_intervalssinjer.png")

```
<br>
<br>
<br>
<br>


## Model 2

```{r echo=FALSE}
knitr::include_graphics("./CC2/jer1/ppc_plot_jer1_mort.png")

```
**Figure 2.** Posterior predictive distribution density plot for deaths.
<br>
<br>
<br>
<br>

```{r echo=FALSE}
knitr::include_graphics("./CC2/jer1/ppc_plot_jer1_hosp.png")

```
<br>
<br>
<br>
<br>


```{r echo=FALSE}
knitr::include_graphics("./CC2/jer1/mu_1_intervalsjer1.png")

```
**Figure 3.** Add caption.
<br>
<br>
<br>
<br>


```{r echo=FALSE}
knitr::include_graphics("./CC2/jer1/beta_intervalsjer1.png")

```
<br>
<br>
<br>
<br>


```{r echo=FALSE}
knitr::include_graphics("./CC2/jer1/beta_h_intervalsjer1.png")

```
<br>
<br>
<br>
<br>


## Model 3

```{r echo=FALSE}
knitr::include_graphics("./CC2/jer2/ppc_plot_jer2_mort.png")

```
**Figure 4.** Posterior predictive distribution density plot for deaths.
<br>
<br>
<br>
<br>


```{r echo=FALSE}
knitr::include_graphics("./CC2/jer2/ppc_plot_jer2_hosp.png")

```
**Figure 5.** Posterior predictive distribution density plot for hospitalizations.
<br>
<br>
<br>
<br>


```{r echo=FALSE}
knitr::include_graphics("./CC2/jer2/mu_1_intervalsjer2.png")

```
**Figure 6.** Add caption 
<br>
<br>
<br>
<br>


```{r echo=FALSE}
knitr::include_graphics("./CC2/jer2/mu_12_intervalsjer2.png")

```
**Figure 7.** Add caption
<br>
<br>
<br>
<br>


```{r echo=FALSE}
knitr::include_graphics("./CC2/jer2/beta_intervalsjer2.png")

```
**Figure 8.** Effect on time from hospitalization to death by covariates included in the third model.
<br>
<br>
<br>
<br>


```{r echo=FALSE}
knitr::include_graphics("./CC2/jer2modi/beta_h_intervalsjer2modi.png")

```
**Figure 9.** Add caption
<br>
<br>
<br>
<br>


## Model 4

```{r echo=FALSE}
knitr::include_graphics("./CC2/jer2modi/ppc_plot_modi_mort.png")

```
<br>
<br>
<br>
<br>


```{r echo=FALSE}
knitr::include_graphics("./CC2/jer2modi/ppc_plot_modi_hosp.png")

```
<br>
<br>
<br>
<br>


```{r echo =FALSE,fig.align='center'}
knitr::include_graphics("CC2/jer2modi/mu_l_intervalsjer2modi.png")

```
<br>
<br>
<br>
<br>


```{r echo =FALSE,fig.align='center'}
knitr::include_graphics("CC2/jer2modi/mu_12_intervalsjer2modi.png")

```
**Figure 10.** Log hazard rate for healthcare provider random effect (95% Credible Interval)
<br>
<br>
<br>
<br>


```{r echo =FALSE,fig.align='center'}
knitr::include_graphics("CC2/jer2modi/beta_intervalsjer2modi.png")

```
<br>
<br>
<br>
<br>


```{r echo =FALSE,fig.align='center'}
knitr::include_graphics("CC2/jer2modi/beta_h_intervalsjer2modi.png")

```
**Figure 11.** Add caption.
