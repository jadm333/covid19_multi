...
data {
  ...
  int<lower=0> Gniv3;                  // num de grupos en niv1
  int<lower=1,upper=Gniv3> Niv3[N];
...
model {
  ...
  target += normal_lpdf(mu_l3_raw | 0, 1);
  ...
  target += reduce_sum(partial_sum,y_mort,grainsize,alpha,(Q_ast*theta +mu_raw_mort+mu_l[Niv1]+mu_l2[Niv2] + mu_l2[Niv3])/alpha);
...
//     mu_l2[Niv2[i]] + mu_l3[Niv3[i]])/alpha));
