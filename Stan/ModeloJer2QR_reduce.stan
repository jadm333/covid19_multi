...
model {
  ...
  target += reduce_sum(partial_sum,y_mort,grainsize,alpha,(Q_ast*theta + mu_raw_mort+mu_l_raw[Niv1]+
  mu_l2_raw[Niv2])/alpha);
...
generated quantities {
  ...
  for(i in 1:N){
    log_lik[i]=weibull_lpdf(y_mort | alpha, exp(-(Q_ast*theta +mu_raw_mort+mu_l_raw[Niv1]+
  mu_l2_raw[Niv2])/alpha))+
    weibull_lpdf(y_hosp | alpha, exp(-(Q_ast_h*theta_h +mu_raw_hosp)/alpha));
    y_mort_tilde[i]=weibull_rng(alpha,exp(-(Q_ast[i]*theta +mu_raw_mort+mu_l_raw[Niv1+
  mu_l2_raw[Niv2]])/alpha));
