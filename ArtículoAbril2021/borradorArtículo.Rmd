...
where $\mathbf{Q}^*$ and $\mathbf{Q}^{**}$ are the orthogonal matrices from the  $\mathbf{QR}$ reparameterization, $\mathbf{\theta}$ and $\mathbf{\vartheta}$ are the coefficient vectors for deaths and hospitalizations, $\mu_m$ and $\mu_h$ represent the global intercepts for deaths and hospitalizations, $alpha$ denotes the shape of the Weibull distribution, and $\alpha_r$  is an extra parameter for the noncentered parameterization This model is described in red in Figure XXX.

### Model II: Two levels

The second model adds an additional level to account for each state of Mexico as random effect to explain deaths, such that, 
$$
\begin{aligned}
...
\end{aligned}
$$
where $\mathbf{\mu}_l, \space l=1,...,32$ represents a local intercept per state (random effect), $\mathbf{\mu}^r_l$ denote the extra paramateres for the noncentered parameterization, and $\sigma$ is the dispersion parameter of the local intercepts. This model is described in light red in Figure XXXX.
...
in which non-informative initial distributions were used, located near \(0\), to improve convergence.
...
```{r,out.width="350px", echo =FALSE}
knitr::include_graphics("mu_12_intervalsjer2.png")
