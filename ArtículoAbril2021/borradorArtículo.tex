...
Fuentes-Garcia Ruth\textsuperscript{\getAff{Science Faculty,
UNAM.}}\textsuperscript{*}\\
\bigskip
\textbf{\getAff{HPME, University of Toronto}}Department 1, Street, City,
State, Zip\\
\textbf{\getAff{Science Faculty, UNAM.}}Department 2, Street, City,
State, Zip\\
\bigskip
...
This model is described in red in Figure XXX.
...
Mexico as random effect to explain deaths, such that, \[
\begin{aligned}
...
intercept per state (random effect), \(\mathbf{\mu}^r_l\) denote the
extra paramateres for the noncentered parameterization, and \(\sigma\)
is the dispersion parameter of the local intercepts. This model is
described in light red in Figure XXXX.
...
in which non-informative initial distributions were used, located near
\(0\), to improve convergence.

To choose the model that best fits the data we considered the
leave-one-out cross-validation (LOO) proposed by {[}17{]}, which
...
population characteristics. \textcolor{red}{JUAN PABLO ESTE NUMERO DE
MUESTRA ES CORRECTO?}
...
disease and 46\% ended up intubated, ventilated or dead {[}19{]}.
